    ...
    public partial class SearchService
    {
        ...
        public IEnumerable<AssociateModel> SearchAssociate(AssociateSearchModel search, int pageSize, int page, string sortColumn, string sortDirection, out int total)
        {
            ...
            var test = associates.ToList();
              // free text
            associates = associates.Where(a => a.AssociateApprovalStatusId<8 || a.AssociateApprovalStatusId>20);
            ...
            if (result.Count() > 0)
            {
                switch (sortColumn)
                {
                    ...
                    case "AssociateId":
                        if (sortDirection == "asc")
                        {
                            ordered = result.OrderBy(x => x.AssociateId);
                        }
                        else
                        {
                            ordered = result.OrderByDescending(x => x.AssociateId);
                        }
                    ...
                    case "Client":
                        if (sortDirection == "asc")
                        {
                            ordered = result.OrderBy(x => x.Client);
                        }
                        else
                        {
                            ordered = result.OrderByDescending(x => x.Client);
                        }
                    ...
                    case "Role":
                        if (sortDirection == "asc")
                        {
                            ordered = result.OrderBy(x => x.Role);
                        }
                        else
                        {
                            ordered = result.OrderByDescending(x => x.Role);
                        }
