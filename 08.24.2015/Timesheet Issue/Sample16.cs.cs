namespace MR_DAL.TimeSheet.Repository
{
    ...
    public class TimeSheetRepository : ITimeSheetRepository
    {
        ...
        public void UpdateTimeSheetEntryNote(int id, string accomodation, string mealAllowance, string travel, string noOfMiles, string parkingNotes, string other)
        {
            if (id<1)
            {
                throw new ArgumentNullException("timeSheetEntry can not be null");
            ...
            var entry = context.TimeSheetEntry.Where(x => x.TimeSheetEntryId == id).First();
            ...
            context.TimeSheets.Context.ObjectStateManager.ChangeObjectState(entry, EntityState.Modified);
            context.SaveChanges();
        }
        ...
        public IEnumerable<ExpensesRecieptDetails> GetRecieptInfo(int roleId, int associateId)
        {
            if (roleId == 0)
            {
            ...
            var result = context.Context.GetExpensesReciept(roleId, associateId).AsEnumerable();
            return result;
        }
